...
function vortex_flow(t::Real, 
    x::Real,
    y::Real;
    strength::Real=1.0,
    left_bound::Real=0.0,
    right_bound::Real=10.0)::(Vector{T} where T <: Real)
    flow = 0.5*(1-tanh(0.1*x^2 + 0.1*y^2 - 10))

    return flow.* [-0.1*x - y; x - 0.1*y]
end

function annulus_flow(t::Real,
    x::Real,
    y::Real ; 
    strength::Real=1.0,
    inner_radius::Real=2.0,
    outer_radius::Real=5.0,
    noise::Real=0.0)::(Vector{T} where T <: Real)
...
function vertical_stream(t::Real, 
    x::Real,
    y::Real;
    strength::Real=1.0,
    left_bound::Real=0.0,
...
function smooth_vertical_stream(t::Real, 
    x::Real,
    y::Real;
    strength::Real=1.0,
    w_1::Real=0.0,
...
function horizontal_stream(t::Real, 
    x::Real,
    y::Real;
    strength::Real=1.0,
    left_bound::Real=0.0,
...
function plot_flow_field(flow_func; t=0, x=-10:1:10, y=-10:1:10)
    ...
    arrows(vec(Point2f.(X, Y)), vec(Point2f.(U, V)), arrowsize=10*n, arrowcolour=n)end
