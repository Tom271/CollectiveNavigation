function get_initial_position(initial_position::String; bounds::Vector{T} where T<: Real=[0,1,0,1], num_agents::Int64=10)
    @assert(length(bounds)==4, "Bounds of initial condition incorrect length")
    initial_position = lowercase(initial_position)

    initial_positions = Dict{String,Any}(
        "box" => box_position(bounds=bounds, num_agents=num_agents)
    )
...
function box_position(; bounds=bounds, num_agents=num_agents)
    ...
    return [rand(Uniform(bounds[1],bounds[2]), (1,num_agents));
            rand(Uniform(bounds[3],bounds[4]), (1,num_agents))]
end

function get_initial_heading(initial_heading::String; bounds::Vector{T} where T<: Real=[0,2π], num_agents::Int64=10)
