...
function find_neighbours_in_range(agent_to_update::Int64, current_pos::Matrix{Float64}; sensing_range::Real=10.0)
    all_distances = calculate_neighbour_distance(agent_to_update, current_pos)
    neighbours = findall(all_distances .< sensing_range)
    return neighbours
end

function find_nearest_neighbours(agent_to_update::Int64, current_pos::Matrix{Float64}; sensing_range::Int=5)
    all_distances = calculate_neighbour_distance(agent_to_update, current_pos)
...
function get_sensing_kernel(sensing::Dict{String, Any})
    ...
    sensing_kernels = Dict{String,Any}(
        "ranged" => (agent_to_update,current_pos) -> find_neighbours_in_range(agent_to_update, current_pos; sensing_range=sensing_range),
        "nearest" => (agent_to_update,current_pos) -> find_nearest_neighbours(agent_to_update, current_pos; sensing_range=sensing_range)
    )
