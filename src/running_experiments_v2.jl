...
Base.@kwdef mutable struct SimulationConfig
    ...
    goal::Dict{String, Any}= Dict(
        "location" => [0.0, 0.0],
        "tolerance" => 5.0)
    information_field_strength::Float64 = 1.0
    flow::Dict{String,Any} = Dict()
    sensing::Dict{String,Any} = Dict(
        "type" => "ranged", 
        "range" => 0.0)
    initial_condition::Dict{String,Any} = Dict(
        "position" => "box",
        "heading" => "sector")
...
config = SimulationConfig(
    num_repeats = 10,
    initial_condition = Dict("position"=>"bOX"),
    sensing = Dict("type"=>"ranged", "range"=>10),
    flow = Dict("type" => "Constant", "strength" => 0.5))

DrWatson.default_allowed(::SimulationConfig) = (Real, String, Symbol, Dict)
DrWatson.allaccess(::SimulationConfig) = ["sensing", "flow"]
DrWatson.default_expand(::SimulationConfig) = ["sensing", "flow"]
...
function run_realisation(config::SimulationConfig; save_output::Bool = false)
    ...
    @unpack flow, sensing, terminal_time, num_agents, kappa_input, kappa_CDF, initial_condition, goal, mean_run_time, α, save_dir,save_name = config
    rng = MersenneTwister()
    ...
    while (majority_travelling(num_agents) &&
        on_track(dist_to_goal) &&
        t <= terminal_time)

        run_time::Float64, agent_to_update = findmin(run_times)
        run_times = run_times .- run_time
        # run all agents 
        current_pos = current_pos  + run_time .* transpose([cos.(current_headings) sin.(current_headings)])
        current_pos = current_pos .+ run_time .* hcat(flow_at.(t,current_pos[1,:], current_pos[2,:])...)
        ...
        goal_direction = atan(reverse(goal["location"] .- current_pos[:, agent_to_update])...)    
        ...
        if num_neighbours > 1
            ...
            ϕ::Float64 = mean(Manifolds.Circle(ℝ), [neighbour_mean, updated_heading], [1-α, α])
            
            weights =  [(1-α)*ones(num_neighbours); α*num_neighbours]
            κ = get_kappa([neighbour_headings; updated_heading], weights, kappa_CDF, kappa_input)
            if κ>1000; println(κ); end
            κ = κ>1000 ? 1000 : κ
        ...
        t = t + run_time
        event_count += 1 
        arrived, dist_to_goal = check_arrivals(current_pos, goal["location"], goal["tolerance"])
...
function get_initial_condition(initial_condition, num_agents)
    initial_pos::Matrix{Float64} = get_initial_position("box"; bounds=[180,220,-20,20], num_agents=num_agents)
    current_headings::Vector{Float64} = get_initial_heading("sector"; bounds=[0,2π], num_agents=num_agents)
...
function run_many_realisations(config)
    ...
    avg_df = combine(gdf, [:average_dist_to_goal, :individuals_remaining, :num_neighbours] .=> mean)
...
function run_experiment(
    ...
    df = DataFrame()
    if default_config.kappa_input ===
        nothing || default_config.kappa_CDF === nothing
