...
parse_config!(config);
#### RUN EXPERIMENT
sensing_values = [0.0, 2.0, 5.0, 10.0, 20.0, 50.0, 500.0];
...
using JLD2
jldsave(datadir("LowFlowHYCOM_actual_ranged.jld2"); arrival_times)
...
save(String(plotsdir("hycom_flow_ranged_actual_low_flow_heatmap_v2.svg")), fig, pt_per_unit=1)
fig
# median_arrival = get_centile_arrival(arrival_times; centile=50);
# full_arrival_times = make_failures_explicit(median_arrival, df, [:sensing_range, :flow_strength]);
# arrival_times = full_arrival_times
