...
ranged_with_flow_data = run_experiment(
    config,
    :flow,
    :sensing;
...
ranged_fig, ax, pltobj = plot_stopping_time_heatmap_v2(
    ranged_with_flow_data,
    :individuals_remaining_mean,
    :flow,
    :sensing,
);
save(plotsdir("ranged_with_flow_ind_rem_heatmap.png"), ranged_fig)
...
parse_config!(config);
nearest_with_flow_df = run_experiment(
    config,
    :flow,
    :sensing;
    flow_values = -0.5:0.1:0.0,
...
nearest_fig, ax, pltobj = plot_stopping_time_heatmap_v2(
    nearest_with_flow_df,
    :individuals_remaining_mean,
    :flow,
    :sensing,
);
save(plotsdir("nearest_with_flow_ind_rem_heatmap.png"), nearest_fig)
