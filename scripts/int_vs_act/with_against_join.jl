...
actual_with_flow_data = run_experiment(
    config,
    :flow,
    :sensing;
    flow_values = -0.5:0.1:0.0,
...
merge(actual_with_flow_data, actual_against_flow_data)
actual_fig, ax, pltobj = plot_stopping_time_heatmap(
    actual_with_flow_data,
    :individuals_remaining_mean,
    :flow,
    :sensing,
);
save(plotsdir("actual_nearest_with_flow_ind_rem_heatmap.png"), actual_fig)
...
intend_with_flow_data = run_experiment(
    config_intend,
    :flow,
    :sensing;
    flow_values = -0.5:0.1:0.0,
...
intend_fig, ax, pltobj = plot_stopping_time_heatmap(
    intend_with_flow_data,
    :individuals_remaining_mean,
    :flow,
    :sensing,
);
save(plotsdir("intend_nearest_with_flow_ind_rem_heatmap.png"), intend_fig)
